...
internal static class AiChatService
{
    ...
    private static async Task<string> SendCodexAsync(DesktopSettings settings, string prompt, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            foreach (var argument in new[] { "exec", "--ephemeral", "--skip-git-repo-check", "--sandbox", "read-only", "--output-last-message", outputPath }) info.ArgumentList.Add(argument);
            if (!string.IsNullOrWhiteSpace(settings.AiModel)) { info.ArgumentList.Add("--model"); info.ArgumentList.Add(settings.AiModel); }
    ...
    private static async Task<string> SendApiAsync(DesktopSettings settings, string prompt, CancellationToken cancellationToken)
    {
        ...
        var body = JsonSerializer.Serialize(new { model = settings.AiModel, messages = new[] { new { role = "user", content = prompt } } });
        using var request = new HttpRequestMessage(HttpMethod.Post, $"{settings.AiApiBaseUrl.TrimEnd('/')}/chat/completions") { Content = new StringContent(body, Encoding.UTF8, "application/json") };
