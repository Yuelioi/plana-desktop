...
The production application uses a transparent Godot Renderer supervised by a .NET Windows host plus a packaged WinUI 3 control center. It includes an IME-capable AI Chat page using a local Codex subscription or an OpenAI-compatible API, semantic character expressions/gestures, user-created Actions, configurable tool groups and interactions, Action Packs, out-of-process Plugins, English/Simplified-Chinese UI, persisted placement/scale, renderer crash recovery, and mouse pass-through. The former Native/WebView and WPF hosts remain as fallback code paths.
