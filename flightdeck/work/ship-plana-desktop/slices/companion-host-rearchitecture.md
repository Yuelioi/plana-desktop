...
The initial full-window implementation was invalid after adding the character polygon: `WS_EX_TRANSPARENT` remained set but Godot's polygon still won hit testing, so the character remained draggable/clickable and user switching could disturb window placement. Full-window mode now travels over the bounded Host→Renderer command channel and is implemented solely by Godot as a tiny polygon entirely outside the window. `tools/check-full-pass-through.ps1` proves command acknowledgement, body-point pass-through, exact position/size stability, and interactive-mode restoration. The Host no longer mutates Godot HWND styles for this mode.
