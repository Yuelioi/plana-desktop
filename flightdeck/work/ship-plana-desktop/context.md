...
- AI conversation is attached to the Companion rather than exposed as a Control Center page: a Host-owned native input field sits below the model for normal focus/IME, and Godot renders temporary thinking/response/error Speech Bubbles near Plana. Settings retains provider/model/API configuration only.
